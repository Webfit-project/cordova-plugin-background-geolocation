...
public class DistanceFilterLocationProvider extends AbstractLocationProvider implements LocationListener {
  ...
  private void setPace(Boolean value) {
    ...
    try {
      log.info("[STP] stop du gps", value);
      locationManager.removeUpdates(this);
      ...
      if (isMoving) {
        ...
        if (!wasMoving) {
          log.info("[STP] isAcquiringSpeed");
          isAcquiringSpeed = true;
        }
      } else {
        log.info("[STP] isAcquiringStationaryLocatio");
      ...
      if (isAcquiringSpeed || isAcquiringStationaryLocation) {
        log.info("[STP] locationAcquisitionAttempts = 0", value);
        ...
        log.info("[STP] else GPS et interval = " + config.getInterval());
        ...
        //locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, config.getInterval(), scaledDistanceFilter, this);
        //locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 0, 0, this);
      }
    } catch (SecurityException e) {
  ...
  public void onLocationChanged(Location location) {
    log.debug("[LOC] Location change: {} isMoving={}", location.toString(), isMoving);
    ...
    if (isAcquiringStationaryLocation) {
      log.debug("[LOC] isAcquiringStationaryLocation");
      ...
      log.debug("[LOC] isAcquiringSpeed");
      if (++locationAcquisitionAttempts == MAX_SPEED_ACQUISITION_ATTEMPTS) {
        ...
        log.debug("[LOC] on aurait du stop pendant l'acquisition del a vitesse mais on le fait pas");
        //return;
      }

    }  else if (isMoving) {
      log.debug("[LOC] isMoving");
      ...
      log.debug("[LOC] stationaryLocation != null");
      return;
    }
